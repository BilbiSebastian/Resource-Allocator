...
class Branch_Name(Master):
    ...
    district = ChainedForeignKey(District, chained_field="state", chained_model_field="name", show_all=False, auto_choose=True, sort=True, limit_choices_to={"isactive": True})
...
class Batch(Master) :
    ...
    def save(self, *args,**kwargs):
        self.batch = f"{self.start_date.strftime('%Y')}_{self.course}_{self.start_date.strftime('%dth %B')}_{self.start_time.strftime('%I:%M %p')}_{self.end_time.strftime('%I:%M %p')}_{self.branch}_{self.Trainer}"
...
class System(Master) :
    ...
    ResponsibleTrainer = models.ForeignKey(User,on_delete=models.CASCADE,verbose_name="Trainer",null=True,blank=False,limit_choices_to={"is_active" : True},related_name='ResponsibleTrainer')
